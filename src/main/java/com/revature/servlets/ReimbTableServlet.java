...
public class ReimbTableServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, 
		...
		switch (tokens[0]) {
		case "manager":
			ManagerHandler(req,resp,om,pw,tokens);
			break;
		case "employee":
			EmployeeHandler(req,resp,om,pw,tokens, username);
			break;
		case "reimb":
			break;
		default:
	...
	private static void ManagerHandler(HttpServletRequest req, 
			...
			PrintWriter prnt, String[] tokens) {
		...
		if (req.getMethod().equals("GET")) {
			if (tokens[1].equals("allreqs")) {
				...
				// Check if further tokens follow /players:
					String jsonReimbData = obm
							.writeValueAsString(m
									.getReimbRequests(username));
