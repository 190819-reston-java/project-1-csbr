...
public class LoginServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try (PrintWriter pw = resp.getWriter()) {
			...
			if (verifyLogin(req, resp, user, username, password, pw)) {
				...
				isManager(isMgr.booleanValue(), user, req, resp);
			} else {
	...
	private static void isManager(boolean isMgr, OrgMember user, HttpServletRequest req, HttpServletResponse resp)
			throws IOException, ServletException {
		if (isMgr && user.isDetermine() == true) {
			req.getServletContext().getRequestDispatcher("/requests_manager.html").forward(req, resp);
		} else if ((isMgr && user.isDetermine() == false) || (!isMgr && user.isDetermine() == true)) {
