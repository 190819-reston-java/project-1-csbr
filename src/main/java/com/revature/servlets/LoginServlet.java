...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (loginVerify(req, resp, user, username, password) && !isManager.booleanValue()) {
			session.setAttribute("user", user);
			req.getServletContext().getRequestDispatcher("/menu.html").forward(req, resp);
		} else if (loginVerify(req, resp, user, username, password) && isManager.booleanValue()) {
			session.setAttribute("user", user);
			req.getServletContext().getRequestDispatcher("/menuEmp.html").forward(req, resp);
		}
